...
class ImageProcessor:
    def __init__(self, rioHei: int = 140):
        ...
        self.line_angle_threshold = 30  # 直线角度阈值(度)
        ...
        self.ImageGetAndVideoStra()

    def ImageGetAndVideoStra(self):
        ...
        # 启动图像获取线程
        self.frame_thread = threading.Thread(target=self.frame_fetcher)
    ...
    def init_debug_windows(self):
        ...
        # 霍夫变换参数滑块
        cv2.createTrackbar('Hough Thresh', 'Road Detection', self.hough_threshold, 200, self.nothing)
        cv2.createTrackbar('Min Line Len', 'Road Detection', self.min_line_length, 200, self.nothing)
        cv2.createTrackbar('Max Line Gap', 'Road Detection', self.max_line_gap, 100, self.nothing)
        cv2.createTrackbar('Line Angle Th', 'Road Detection', self.line_angle_threshold, 90, self.nothing)
    ...
    def update_parameters(self):
        ...
        # 更新霍夫变换参数
        self.hough_threshold = cv2.getTrackbarPos('Hough Thresh', 'Road Detection')
        self.min_line_length = cv2.getTrackbarPos('Min Line Len', 'Road Detection')
        self.max_line_gap = cv2.getTrackbarPos('Max Line Gap', 'Road Detection')
        self.line_angle_threshold = cv2.getTrackbarPos('Line Angle Th', 'Road Detection')

        if self.morph_size % 2 == 0:
            self.morph_size += 1

    def detect_road_boundaries(self, mask, roi_width, roi_height):
        """使用霍夫变换检测道路边界线"""
        # 边缘检测增强
        ...
        def average_line(lines):
            if not lines:
                return None
            xs = np.array([l[0] for l in lines] + [l[2] for l in lines])
            ys = np.array([l[1] for l in lines] + [l[3] for l in lines])
    ...
    def calculate_road_centerline(self, left_line, right_line, roi_width, roi_height):
        """根据左右边界线计算道路中轴线"""
        if left_line is None and right_line is None:
            return None, 0, None, None

        # 如果只有一侧的边界线，使用历史角度估算另一侧
        if left_line is None:
            # 使用历史角度估算左侧边界
            angle_rad = math.radians(self.analysis_result['road_angle'])
            dx = self.roi_height * math.tan(angle_rad)
            x1 = roi_width // 2 - int(dx)
            x2 = roi_width // 2 - int(dx)
            left_line = (x1, 0, x2, roi_height)
        ...
        if right_line is None:
            ...
            x1 = roi_width // 2 + int(dx)
            x2 = roi_width // 2 + int(dx)
            right_line = (x1, 0, x2, roi_height)

        # 计算左右边界在底部的点
        left_bottom = (left_line[2], left_line[3])
        right_bottom = (right_line[2], right_line[3])

        # 计算道路底部中心点
        bottom_center = ((left_bottom[0] + right_bottom[0]) // 2, left_bottom[1])

        # 计算左右边界在顶部的点
        left_top = (left_line[0], left_line[1])
        right_top = (right_line[0], right_line[1])

        # 计算道路顶部中心点
        top_center = ((left_top[0] + right_top[0]) // 2, left_top[1])
        ...
        # 计算中轴线端点
        start_point = (top_center[0], top_center[1])
        end_point = (bottom_center[0], bottom_center[1])

        return (start_point, end_point), angle, left_line, right_line if left_line and right_line else None
    ...
    def process_frame(self, frame):
        ...
        # 检测道路边界
        left_line, right_line = self.detect_road_boundaries(mask_processed, roi_width, roi_height)
        ...
        if current_time - self.analysis_result['last_angle_update'] > self.ANGLE_UPDATE_INTERVAL:
            centerline, angle, left_line, right_line = self.calculate_road_centerline(
                left_line, right_line, roi_width, roi_height
            )
            ...
            self.analysis_result['left_line'] = left_line
            self.analysis_result['right_line'] = right_line
        ...
        # 绘制左右边界线
        if self.analysis_result['left_line']:
            x1, y1, x2, y2 = self.analysis_result['left_line']
            cv2.line(roi_draw, (x1, y1), (x2, y2), (0, 255, 255), 2)

        if self.analysis_result['right_line']:
            x1, y1, x2, y2 = self.analysis_result['right_line']
            cv2.line(roi_draw, (x1, y1), (x2, y2), (0, 255, 255), 2)

        # 计算位置偏移量
        center_x = roi_width // 2

        # 关键修复：检查centerline是否为None
        if centerline:
            bottom_center = centerline[1]
            position_offset = bottom_center[0] - center_x
            ...
            # 当无法检测到道路中线时，使用默认值
            self.analysis_result['position_offset'] = 0
            self.analysis_result['action'] = "前进"
            self.analysis_result['action_code'] = "forward"

        # 绘制中心线
        cv2.line(roi_draw, (center_x, 0), (center_x, roi_height), (255, 255, 0), 2)
    ...
    try:
        while True:
            ...
            if angle_history:
                ...
                # 选择更稳定的角度值
                if abs(Average_Deg - Median_Deg) < 5:
                    smoothed_angle = (Average_Deg + Median_Deg) / 2
                elif abs(Average_Deg) < abs(Median_Deg):
                    smoothed_angle = Average_Deg
                else:
                    smoothed_angle = Median_Deg
            else:
                smoothed_angle = current_angle if current_angle is not None else 0

            print(analysis_result['action_code'])
            ...
            if not robot.is_moving:
                ...
                if (abs(smoothed_angle) > DegWait and
                    ...
                    rotation_duration = min(0.2, abs(smoothed_angle) / 200.0)  # 限制最大旋转时间
            ...
            # 检查偏移量和角度是否在允许范围内
            angle_stable = analysis_result['angle_stable']
        ...
        # 清理资源
        if Angle_History_All:
            Pandas = pd.Series(Angle_History_All)
            print(f"全部角度：{Angle_History_All}\n"
                  f"最大值:{round(max(Angle_History_All), 2)}\n "
...
if __name__ == "__main__":
    ...
    class ROISelector:
        ...
        def mouse_callback(self, event, x, y, flags, param):
            if event == cv2.EVENT_LBUTTONDOWN:
                ...
                self.roi = (min(x1, x2), min(y1, y2),
                            abs(x2 - x1), abs(y2 - y1))
                print(f"ROI选择完成: {self.roi}")


    # 使用示例
    selector = ROISelector()
    img = cv2.imread("开发档案/img_1.png")

    cv2.namedWindow("ROI Selector")
    cv2.setMouseCallback("ROI Selector", selector.mouse_callback)
    ...
    while True:
        ...
        if selector.start_point and selector.end_point:
            cv2.rectangle(display_img, selector.start_point,
                          selector.end_point, (0, 255, 0), 2)

        cv2.imshow("ROI Selector", display_img)

        key = cv2.waitKey(1) & 0xFF
        if key == ord('s') and selector.roi:
            # 保存ROI区域
            x, y, w, h = selector.roi
            roi_img = img[y:y + h, x:x + w]
            cv2.imwrite("roi_selected.jpg", roi_img)
            print("ROI已保存")
        elif key == 27:
            break
